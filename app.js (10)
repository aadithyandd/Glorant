...
/* =================================================================
   10. MULTIPLAYER ROOMS, AUTO-CLEANUP & RESPAWN
   ================================================================= */
...
async function joinRoom(roomId, name) {
  ...
  // AUTO-CLEANUP: Remove player on disconnect, and clear room if empty
  onDisconnect(playerRef).remove().then(() => {
    get(ref(db, `rooms/${roomId}/players`)).then((snap) => {
      ...
      if (!remaining || Object.keys(remaining).length <= 1) {
        remove(roomRef); // Delete entire room from DB and clear cache when inactive
      }
    });
  });
  ...
  onValue(ref(db, `rooms/${roomId}/players`), (snap) => {
    ...
    // Track active remote IDs to clean up stale meshes
    const activeIds = Object.keys(list);
    ...
    for (let id of activeIds) {
      ...
      pMesh.visible = true; // Force visibility check on every frame sync
    ...
    // Clean up disconnected or removed remote players from scene
    for (let id in remotePlayers) {
      if (!list[id]) {
