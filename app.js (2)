...
function executeSingleShot() {
  ...
  // Online Hitscan & Bullet Broadcast
  const activeTargets = Object.values(remotePlayers).filter(m => !m.isDead);
  ...
  // Broadcast bullet path and sound to other players in the room
  if (currentRoom) {
...
async function joinRoom(roomId, name) {
  ...
  // Listen for incoming bullets from other players in the room
  onChildAdded(ref(db, `rooms/${roomId}/shots`), (snap) => {
    const shot = snap.val();
    if (!shot || shot.id === player.id) return; // Ignore own shots
    ...
    // Play enemy gun audio depending on weapon type
    if (shot.weapon === 'sheriff') SoundFx.sheriffShoot();
    else SoundFx.shoot();

    // Render tracer from enemy barrel
