...
/* =================================================================
   4. WEAPON RIGS (RIFLE, KARAMBIT, SHERIFF)
   ================================================================= */
...
/* =================================================================
   5. ORIGINAL FLAT-GROUND MAP & COLLIDERS
   ================================================================= */
const colliders = [];
...
/* =================================================================
   6. BULLET TRACERS (LESS VISIBLE / REDUCED OPACITY FOR CLUTTER REDUCTION)
   ================================================================= */
const tracers = [];
function spawnTracer(startVec, endVec) {
  const geom = new THREE.BufferGeometry().setFromPoints([startVec, endVec]);
  // Reduced opacity and subtle line width for less clutter
...
// Scoreboard Modal Logic (T Key on PC, Tap Anywhere to Close on Mobile)
...
// Mobile: Tap anywhere on scoreboard overlay to close it
sbModal.addEventListener('touchstart', (e) => {
  e.preventDefault();
  if (isScoreboardOpen) toggleScoreboard();
}, { passive: false });
...
// Mobile Controls (Fixed Fire Button Drag-Aiming)
let joyTouchId = null;
...
/* =================================================================
   10. MULTIPLAYER ROOMS, AUTO-CLEANUP & RANDOM SPAWNS
   ================================================================= */
...
function getRandomSpawnPosition() {
  ...
  return {
    x: THREE.MathUtils.clamp(Math.cos(angle) * dist, -MAP_BOUND, MAP_BOUND),
    z: THREE.MathUtils.clamp(Math.sin(angle) * dist, -MAP_BOUND, MAP_BOUND)
  };
}
...
function respawn() {
  ...
  camera.position.set(spawnPos.x, STAND_EYE_HEIGHT, spawnPos.z);
...
async function joinRoom(roomId, name) {
  ...
  onDisconnect(playerRef).remove().then(() => {
    get(ref(db, `rooms/${roomId}/players`)).then((snap) => {
      ...
      if (!remaining || Object.keys(remaining).length <= 1) {
        remove(roomRef);
      }
    });
  });
